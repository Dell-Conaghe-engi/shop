...
from database.utils import db_get_product_by_id, db_get_product_by_name, db_get_user_cart, db_add_or_update_item
...
async def change_product_quantity(callback: CallbackQuery, bot: Bot):
    '''меняем количество товаров в карзине'''
    ...
    result = db_add_or_update_item(
        ...
        increment = increment)
    if result["status"] == "errror":
        await callback.answer('ошибка при изменении данных', show_alert=True)
        return
